...
pub fn resolve_secret<T: std::str::FromStr<Err = ParseError>>(
    ...
    if stdin_flag {
        ...
        return line.trim().parse::<T>().map_err(KdubError::Parse);
    }
    ...
    if batch {
        return Err(KdubError::NotImplemented(
            "batch mode requires secret via flag, stdin, or env var",
        ));
    }
...
fn read_line_from_stdin() -> Result<String, KdubError> {
