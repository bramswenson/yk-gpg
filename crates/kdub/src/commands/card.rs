...
use kdub_lib::card::{
    CardExecutor, CardInfo, CardKeyInfo, KeySlot, PcscCardExecutor, TouchPolicyInfo, require_tty,
};
...
use kdub_lib::types::{AdminPin, Passphrase, UserPin};
...
fn run_setup(args: &CardSetupArgs, global: &GlobalOpts) -> CmdResult {
    ...
    // Resolve new PINs (validate if provided, otherwise will be generated)
    let new_admin_pin = args
        .new_admin_pin
        .as_ref()
        .map(|s| {
            s.parse::<AdminPin>()
    ...
    println!("  User PIN:   {}", result.user_pin.expose_secret());
    println!();
...
fn run_provision(args: &CardProvisionArgs, global: &GlobalOpts) -> CmdResult {
    ...
    for slot in &subkey_slots {
        let slot_name = match slot.slot {
            KeySlot::Signing => "Signing (slot 1)",
            KeySlot::Decryption => "Decryption (slot 2)",
            KeySlot::Authentication => "Authentication (slot 3)",
        };
        println!(
            "    [{}] -> {}",
...
///
/// Same logic as in `commands/key.rs` — shared pattern for card commands.
...
/// Normalize a key ID: strip `0x` prefix, uppercase.
