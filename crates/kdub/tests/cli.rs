...
fn version_shows_build_info() {
    Command::cargo_bin("kdub")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("kdub 0.1.0"))
...
fn init_with_tor_proxy_creates_dirmngr() {
    ...
    Command::cargo_bin("kdub")
        ...
        .env("XDG_DATA_HOME", tmp.path().join("data_home"))
        .env("XDG_STATE_HOME", tmp.path().join("state_home"))
        .env("KDUB_TOR_PROXY", "socks5://127.0.0.1:9050")
        .assert()
        .success();
...
fn completions_bash_produces_output() {
    Command::cargo_bin("kdub")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("_kdub"))
...
fn completions_zsh_produces_output() {
    Command::cargo_bin("kdub")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("#compdef kdub"));
}
