...
impl CardExecutor for PcscCardExecutor {
    fn card_info(&mut self) -> Result<CardInfo, KdubError> {
        ...
        // KDF
        let kdf_enabled = tx.kdf_do().is_ok();
    ...
    fn import_key(
        ...
        // The trait's byte-based interface exists for mockability. In
        // production, callers should use `PcscCardExecutor::import_subkey`
        // which accepts the rPGP `SecretSubkey` directly, avoiding the
        // need to serialize/deserialize individual OpenPGP packets.
        //
        // This method delegates to import_subkey — but since we cannot
        // easily reconstruct a SecretSubkey from raw bytes without the
        // packet header, we provide a concrete method for real callers.
        Err(KdubError::Card(format!(
            "use PcscCardExecutor::import_subkey for slot {slot:?} (byte-based import not supported; \
             admin_pin({:?}) and passphrase provided)",
            admin_pin,
        )))
    }
