...
pub struct NetworkConfig {
    /// SOCKS5 proxy for Tor routing (e.g., "socks5://127.0.0.1:9050").
    #[serde(default)]
...
"#;
        ...
        assert_eq!(config.network.tor_proxy, "socks5://127.0.0.1:9050");
...
"#;
    ...
    fn test_env_var_overlay_tor_proxy() {
        // SAFETY: test-only, single-threaded test execution
        unsafe { std::env::set_var("KDUB_TOR_PROXY", "socks5://localhost:9050") };
        let config = KdubConfig::load(None).unwrap();
        assert_eq!(config.network.tor_proxy, "socks5://localhost:9050");
