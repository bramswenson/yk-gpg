...
mod tests {
    ...
    fn test_renew_updates_expiration() {
        ...
        // Original key has 3 subkeys with no explicit expiration set at generation time
        assert_eq!(key.secret_subkeys.len(), 3);

        // Renew with 3y expiration
        let (renewed, actions) = renew_subkeys(&key, "3y", "testpassphrase").unwrap();
    ...
    fn test_renew_with_never_expiration() {
        ...
        let (renewed, actions) = renew_subkeys(&key, "never", "testpassphrase").unwrap();
    ...
    fn test_renew_preserves_key_flags() {
        ...
        let (renewed, _) = renew_subkeys(&key, "1y", "testpassphrase").unwrap();
    ...
    fn test_renew_invalid_expiration() {
        ...
        let result = renew_subkeys(&key, "2w", "testpassphrase");
    ...
    fn test_run_renew_full_workflow() {
        ...
        // Run renewal
        let actions = run_renew(data_dir, &fp, "2y", "testpassphrase").unwrap();
        assert_eq!(actions.len(), 3);
    ...
    fn test_renew_refuses_stubbed_key() {
        ...
        let result = run_renew(data_dir, &fp, "2y", "testpassphrase");
