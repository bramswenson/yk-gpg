...
pub fn run_restore(data_dir: &Path, fingerprint: &str) -> Result<Vec<String>, KdubError> {
    let backup_dir = data_dir.join("backups").join(fingerprint);
    if !backup_dir.exists() {
        return Err(KdubError::BackupNotFound(fingerprint.to_string()));
    }
    ...
    let key_path = identities_dir.join(format!("{fingerprint}.key"));
    ...
    let metadata_path = identities_dir.join(format!("{fingerprint}.json"));
    if metadata_path.exists() {
        // Update existing metadata
        let mut meta = IdentityMetadata::load(data_dir, fingerprint)?;
        meta.backed_up = Some(chrono::Utc::now());
        meta.save(data_dir)?;
...
pub fn load_key_from_store(
    ...
    if !key_path.exists() {
        return Err(KdubError::KeyNotFound(fingerprint.to_string()));
    }
...
mod tests {
    ...
    fn generate_test_key() -> (SignedSecretKey, String) {
        ...
        .unwrap();
        let fp = keygen::extract_fingerprint(&key);
        (key, fp)
    }
    ...
    fn test_backup_creates_all_files() {
        ...
        let actions = run_backup(data_dir, &fp, &key, Some("testpassphrase")).unwrap();
        assert_eq!(actions.len(), 5);

        let backup_dir = data_dir.join("backups").join(&fp);
    ...
    fn test_backup_file_permissions() {
        ...
        run_backup(data_dir, &fp, &key, Some("testpassphrase")).unwrap();
    ...
    fn test_backup_public_key_content() {
        ...
        let public_key_content =
            fs::read_to_string(data_dir.join("backups").join(&fp).join("public-key.asc")).unwrap();
        assert!(
    ...
    fn test_backup_certify_key_content() {
        ...
        let certify_content =
            fs::read_to_string(data_dir.join("backups").join(&fp).join("certify-key.asc")).unwrap();
        assert!(
    ...
    fn test_backup_ownertrust_format() {
        ...
        let ownertrust_content =
            fs::read_to_string(data_dir.join("backups").join(&fp).join("ownertrust.txt")).unwrap();
    ...
    fn test_backup_restore_roundtrip() {
        ...
        assert!(
            loaded_meta.backed_up.is_some(),
            "backed_up timestamp should be set"
        );
    }
    ...
    fn test_restore_missing_backup() {
        ...
        let result = run_restore(data_dir, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
        assert!(result.is_err());
        match result.unwrap_err() {
            KdubError::BackupNotFound(fp) => {
                assert_eq!(fp, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
            }
    ...
    fn test_restore_creates_metadata_when_missing() {
        ...
        assert_eq!(meta.identity, "Backup Test <backup@example.com>");
    }
    ...
    fn test_load_key_from_store_not_found() {
        let tmp = tempfile::tempdir().unwrap();
        let result = load_key_from_store(tmp.path(), "NOEXIST");
        assert!(result.is_err());
        match result.unwrap_err() {
            KdubError::KeyNotFound(fp) => assert_eq!(fp, "NOEXIST"),
