...
pub fn export_armored_pubkey(data_dir: &Path, fingerprint: &str) -> Result<String, KdubError> {
...
pub fn load_public_key(data_dir: &Path, fingerprint: &str) -> Result<SignedPublicKey, KdubError> {
    let key = crate::backup::load_key_from_store(data_dir, fingerprint)?;
...
/// the specified SOCKS proxy (e.g., `socks5://127.0.0.1:9050`).
...
mod tests {
    ...
    fn test_make_http_agent_with_proxy() {
        ...
        config.network.tor_proxy = "socks5://127.0.0.1:9050".to_string();
        let agent = make_http_agent(&config);
    ...
    fn test_make_http_agent_with_proxy_string() {
        ...
        let mut config = KdubConfig::default();
        config.network.tor_proxy = "socks5://192.168.1.1:1080".to_string();
        let agent = make_http_agent(&config);
    ...
    fn test_publish_to_wkd() {
        ...
        let result = publish_to_wkd(tmp.path(), "joe@example.com", &public_key);
        assert!(result.is_ok(), "WKD publish should succeed: {result:?}");

        // Verify the WKD file was created
        let (hash, domain) = wkd::wkd_hash("joe@example.com").unwrap();
        let wkd_path = tmp
            .path()
            .join(".well-known")
            .join("openpgpkey")
