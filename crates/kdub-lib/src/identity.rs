...
pub struct IdentityMetadata {
    ...
    pub identity: String,
    /// Hex-encoded primary key fingerprint (uppercase, 40 chars for V4)
...
impl IdentityMetadata {
    ...
    pub fn load(data_dir: &Path, fingerprint: &str) -> Result<Self, KdubError> {
        let path = data_dir
            .join("identities")
...
pub fn find_identity(data_dir: &Path, query: &str) -> Result<IdentityMetadata, KdubError> {
    ...
    // 1. Exact fingerprint match (case-insensitive)
    let exact: Vec<_> = all.iter().filter(|m| m.fingerprint == query_hex).collect();
    if exact.len() == 1 {
    ...
    if query_hex.len() >= 8 && query_hex.chars().all(|c| c.is_ascii_hexdigit()) {
        let prefix_matches: Vec<_> = all
            .iter()
            .filter(|m| m.fingerprint.starts_with(query_hex))
            .collect();
    ...
    if query_hex.len() >= 8 && query_hex.chars().all(|c| c.is_ascii_hexdigit()) {
        let keyid_matches: Vec<_> = all
            .iter()
            .filter(|m| m.fingerprint.ends_with(query_hex))
            .collect();
...
mod tests {
    ...
    fn test_metadata_load_missing_file() {
        let tmp = tempfile::tempdir().unwrap();
        let result = IdentityMetadata::load(tmp.path(), "NOEXIST");
